...
func SM4Encrypt(key, src []byte) ([]byte, error) {
	...
	// if len(src) < 16 {
	// for l := len(src); l < 16; l++ {
	// 	src = append(src, 0)
	// 	// return nil, errors.New("the length of src param needed be more than 16")
	// }
	dst := make([]byte, len(src))
	fmt.Println("EncryptBlock:", key, dst, src)
...
func (*gmsm4Encryptor) Encrypt(k bccsp.Key, plaintext []byte, opts bccsp.EncrypterOpts) (ciphertext []byte, err error) {
	fmt.Println("Encrypt:", k, plaintext, opts)
